...
- The feature directory (e.g. `features/difficulty-rework/`). Used to locate the artifacts and to build the links in the PR body.
...
3. `implementation-report.md` — pull the Summary (for the PR summary) and the Tests section (for the PR's Tests section). When drafting the PR summary, rewrite any jargon-heavy sentences: no function names, variable names, or other hyper-specific technical detail. Describe behaviour and outcomes only.
4. `implementation.md` — skim Approach & Key Decisions for additional context if the report Summary is thin.
5. **Key files** — run `git diff main...HEAD --name-only` to see every changed file. From that list (and from the plan/report), identify the 1–3 files a human reviewer must read to understand what the PR actually does. Prefer production code over tests and generated files. For each chosen file:
...
[One paragraph describing what the feature does and why it matters, written for a developer who is broadly familiar with the project but has not read any branch artifact. Use plain language: describe behaviour and user-visible outcomes, not implementation details. No function names, variable names, or other hyper-specific technical jargon — those belong in the Key files section or the artifacts. Seed the content from `implementation-report.md`'s feature summary, but rewrite any jargon-heavy sentences before including them.]
...
- [✅ | ⚠️ | ❌] <AC1 text, expressed in plain behavioural terms — no function names or file paths>
- [✅ | ⚠️ | ❌] <AC2 text>
- ...
...
- `<file>` (<N> tests): <one-line coverage summary>
- ... (one bullet per test file)
...
The Tests section headline (`N tests added · all passing`) must be accurate: count every new test function across all test files touched, then confirm all pass. The detail block is lifted straight from `implementation-report.md`'s Tests section. If there were no test changes, omit the section entirely.

When writing Acceptance Criteria bullets, describe observable behaviour only — what the system does or does not do. Never name specific functions, parameters, or file paths in the checklist itself; those belong in the Evidence detail block.
...
- AC checklist bullets must describe observable behaviour only — no function names, parameters, or file paths. Move any such specifics to the Evidence detail block.
