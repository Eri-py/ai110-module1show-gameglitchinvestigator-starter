...
1. Ask the user which base branch to branch from. Do not assume `main` or `master`.
2. Confirm the working tree is clean (`git status`). If not, stop and ask the user to commit or stash first.
3. Ask the user what branch-naming convention this repo uses (check recent branch names or the project's instruction file if unsure), and propose a branch name that follows it, seeded from the plan directory name (e.g. a plan in `features/difficulty-rework/` → `feature/difficulty-rework`, or the project's own prefix in place of `feature/`). For nested sub-plans, include the parent directory (e.g. `feature/difficulty-rework-01-range-fix`). Confirm the name with the user before creating.
...
2. Verify the previous task was fully successful — all its success criteria met, tests passing. If not, stop and report.
...
- **Tests:** test files added or modified. For each: filename, test count, and a one-line description of what it covers. End the section with a single line confirming all tests pass (guaranteed by the per-task test gate in Step 4).
...
- Never continue after a failing test, or a skipped test.
